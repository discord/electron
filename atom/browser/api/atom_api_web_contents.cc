    ...
    if (mate::ConvertFromV8(isolate, input_event, &mouse_event)) {
      ...
      if (type == blink::WebInputEvent::kMouseMove) {
        if (!dragging && start_dragging) {
          ...
          if (bitmap) {
            mate::Handle<NativeImage> image = NativeImage::Create(
                isolate, gfx::Image::CreateFrom1xBitmap(*bitmap));
            Emit("start-drag", image,
...
void WebContents::OnPaint(const gfx::Size& size,
  ...
  overlay_.SendFrame(size, dirty_rect, std::move(unsafe_shm), std::move(read_shm), std::move(done_cb));
}
